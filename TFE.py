...
class TweetsFiles :
    '''
    This class is used to store the tweets in plain text files, one tweet for each row,
    automatically splitting them in different files of up to 'limit' tweets each.
    
    An instance of TweetsFiles requires a destination folder, it searches for previously
    created files and continues from the point it was stopped.

    Each filename has the following format:

    {NAME}_{N}.{EXTENSION}

    Where N is an increasing natural number.
...
class TFEStream(TwythonStreamer):
    '''
    Simple stream listener that store tweets in plain json format,
    using TweetsFile to split them in different files.

    '''
    def __init__(self,c1,c2,c3,c4,tweets_folder,max_errors) :
        super(TFEStream, self).__init__(c1,c2,c3,c4)
